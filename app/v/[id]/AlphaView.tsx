...
export function AlphaView({ skin, signal }: Pick<ScreenConfig, "skin" | "signal">) {
  return (
    <div className={`${styles.shell} ${styles[skin]}`}>
      <aside className={styles.sidebar}>
        ...
        <nav aria-label="Workspace navigation" className={styles.navigation}>
          ...
          {navItems.map((item) => (
            <a href={`#${item.toLowerCase().replace(" ", "-")}`} className={item === "Projects" ? styles.current : undefined} key={item}>
      ...
      <main className={styles.main}>
        <header className={styles.header}>
          ...
          <div className={styles.headerMeta}><span className={styles.search}>⌕&nbsp;&nbsp; Search projects</span><span className={styles.date}>September 2026</span><span className={styles.headerAvatar}>MK</span></div>
        </header>
            ...
            <div className={styles.projectList}>
              {projects.map((project) => (
                <article className={styles.project} key={project.code}>
                  ...
                  <ActionButton signal={signal} label={`Open actions for ${project.name}`} className={`${styles.action} ${signal === 1 ? styles.quartz : ""}`} iconClassName={styles.menuIcon} />
                </article>
              ))}
            </div>
          </section>
