...
export function BetaView({ skin, signal }: Pick<ScreenConfig, "skin" | "signal">) {
  return (
    <main className={`${styles.page} ${styles[skin]}`}>
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <a className={styles.brand} href="#top"><span aria-hidden="true">L</span>Leafline</a>
          <nav aria-label="Main navigation"><a className={styles.current} href="#catalogue">Catalogue</a><a href="#paths">Learning paths</a><a href="#teams">For teams</a><a href="#resources">Resources</a></nav>
          <div className={styles.account}><span className={styles.search}>Search</span><span className={styles.avatar}>AR</span><span><strong>Alex Rivera</strong><small>3 courses active</small></span></div>
        </div>
      </header>
      ...
      <section className={styles.intro} id="top">
        <div className={styles.introInner}>
          ...
          <div className={styles.introCopy}><p>Practical courses created with experienced leaders, makers and specialists. Learn something useful today, then put it to work tomorrow.</p><div className={styles.proof}><span><strong>48</strong> expert-led courses</span><span><strong>4.9</strong> average rating</span></div></div>
        </div>
      </section>
      ...
      <div className={styles.catalogue} id="catalogue">
        ...
        <nav className={styles.filters} aria-label="Course categories">{categories.map((category) => <a className={category === "All courses" ? styles.selected : undefined} href={`#${category.toLowerCase().replace(" ", "-")}`} key={category}>{category}</a>)}</nav>
          ...
          {courses.map((course) => (
            <article className={styles.card} key={course.title}>
              <div className={`${styles.cover} ${styles[course.color]}`}>
                ...
                <ActionButton signal={signal} label={`Bookmark ${course.title}`} className={`${styles.bookmark} ${signal === 1 ? styles.quartz : ""}`} iconClassName={styles.bookmarkIcon} />
